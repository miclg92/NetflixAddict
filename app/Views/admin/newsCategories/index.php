...
if (isset($_SESSION['auth']) && isset($_SESSION['user']) && $_SESSION['user']->flag == 2) {
	...
	<div id="bloc_content">
		<div id="admin_categories" class="form">
			...
			<a href="?p=admin.newsCategories.add" type="button" id="login_form_btn" class="btn col-xs-12"
			   aria-hidden="true"><i class="fas fa-plus" aria-hidden="true"></i> Ajouter une Catégorie</a>
			<table class="table">
				<thead>
				<tr>
					<td>TITRE</td>
				...
				<?php foreach ($categories as $category): ?>
					<tr>
						...
						<td id="buttons-actions">
							<a class="btn" href="?p=admin.newsCategories.edit&id=<?= $category->id; ?>"><i
										class="fas fa-pencil-alt" aria-hidden="true"></i> Modifier</a>
							<form action="?p=admin.newsCategories.delete" method="post"
							      onclick="return confirm_delete();" style="display: inline;">
								<input type="hidden" name="id" value="<?= $category->id ?>">
								<button type="submit" class="btn"><i class="fas fa-trash-alt" aria-hidden="true"></i>
									Supprimer
								</button>
							</form>
						</td>
